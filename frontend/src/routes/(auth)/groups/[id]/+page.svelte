<script lang="ts">
    ...
    async function renameSchedule(scheduleId: string, oldTitle: string) {
        ...
        if (newTitle && newTitle !== oldTitle && schedule) {
            try {
    ...
    function handleCellClick(d: number, t: number, scheduleStart: string) {
        ...
        const availMembers = rawAvailability
            .filter(userAvail => userAvail.blocks.some(b => new Date(b.start).getTime() === targetTime))
            .map(ua => {
                                ...
                                <div class="card-body p-4 sm:p-5">
                                        ...
                                        <div class="w-full">
                                            <div class="flex justify-between sm:justify-start items-center gap-2">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <div class="badge {schedule.status === 'ACTIVE' ? 'badge-success text-white' : 'badge-neutral'}">{schedule.status}</div>
                                                    {#if isAdmin}
                                    ...
                                    {#if openHeatmapId === schedule.id && heatmapData}
                                        <div class="mt-4 relative bg-base-200 rounded-lg overflow-hidden border border-base-300">
                                            ...
                                            {#if selectedCell}
                                                <div class="absolute inset-y-0 right-0 w-48 bg-base-100 shadow-2xl border-l border-base-300 z-20 flex flex-col">
